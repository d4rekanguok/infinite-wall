...
export default class {
  constructor($wall) {
    ...
    hammer.on("panend", e => {
      ...
      const { snap, gap, offsetX, offsetY } = this.gridSize;
      const { w: slotW, h: slotH } = this.slotSize;

      // total distance from original position
      let distanceY = y - deltaY;
      let distanceX = x - deltaX;

      if (snap) {
        distanceY = (slotH + gap) * Math.round(distanceY / (slotH + gap)) - (offsetY * slotH);
        distanceX = (slotW + gap) * Math.round(distanceX / (slotW + gap)) - (offsetX * slotW);
      }

      // TODOS: take this out into a function
      // loop content
      contentRootPos.y = distanceY % contentTotalHeight;
      contentRootPos.x = distanceX % contentTotalWidth;
  ...
  snap($slot, setting) {
    let start = false;
    const { fromPos, toPos, duration } = setting;

    function step(timestamp) {
      if (!start) start = timestamp;
      const delta = timestamp - start;
      const x = fromPos.x + (toPos.x - fromPos.x) / duration * delta;
      const y = fromPos.y + (toPos.y - fromPos.y) / duration * delta;
      $slot.style = `transform: translate(${x}px, ${y}px);`;
      if (delta < duration) window.requestAnimationFrame(step);
    }
  }
